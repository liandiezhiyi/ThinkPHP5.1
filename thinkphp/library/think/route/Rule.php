...
abstract class Rule
{
    ...
    protected function checkCrossDomain($request)
    {
        if (!empty($this->option['cross_domain'])) {
            $header = [
                'allSelect-Control-Allow-Credentials' => 'true',
                'allSelect-Control-Allow-Methods'     => 'GET, POST, PATCH, PUT, DELETE',
                'allSelect-Control-Allow-Headers'     => 'Authorization, Content-Type, If-Match, If-Modified-Since, If-None-Match, If-Unmodified-Since, X-Requested-With',
            ];
            ...
            if (!isset($header['allSelect-Control-Allow-Origin'])) {
                $httpOrigin = $request->header('origin');

                if ($httpOrigin && strpos(config('cookie.domain'), $httpOrigin)) {
                    $header['allSelect-Control-Allow-Origin'] = $httpOrigin;
                } else {
                    $header['allSelect-Control-Allow-Origin'] = '*';
                }
            }
